...
const ENV_KEYS = [
  "MEMORY_CIP_OAUTH_AUTHORIZE_URL",
  "MEMORY_CIP_OAUTH_TOKEN_URL",
  "MEMORY_CIP_OAUTH_REDIRECT_URI",
  "MEMORY_CIP_OAUTH_CLIENT_ID",
  "OPENCLAW_HOME",
];
...
describe("memory-cip auth", () => {
  ...
  it("round-trips a dedicated llm api-key config through OAuth login/logout", async () => {
    ...
    process.env.MEMORY_CIP_OAUTH_AUTHORIZE_URL = `http://127.0.0.1:${tokenPort}/oauth/authorize`;
    process.env.MEMORY_CIP_OAUTH_TOKEN_URL = `http://127.0.0.1:${tokenPort}/oauth/token`;
    process.env.MEMORY_CIP_OAUTH_REDIRECT_URI = `http://localhost:${redirectPort}/auth/callback`;
    process.env.MEMORY_CIP_OAUTH_CLIENT_ID = "test-client-id";

    const configPath = path.join(tempDir, "openclaw.json");
    const oauthPath = path.join(tempDir, ".memory-lancedb-cip", "oauth.json");
    ...
    createMemoryCLI({
      ...
      oauthTestHooks: {
        authorizeUrl: async (url) => {
          ...
          setTimeout(() => {
            const callback = new URL(process.env.MEMORY_CIP_OAUTH_REDIRECT_URI);
    ...
    const pluginConfig = updatedConfig.plugins.entries["memory-lancedb-cip"].config;
    ...
    assert.match(output, /Updated memory-lancedb-cip config: llm.auth=oauth, llm.oauthProvider=openai-codex/);
    ...
    const restoredPluginConfig = restoredConfig.plugins.entries["memory-lancedb-cip"].config;
    assert.deepEqual(restoredPluginConfig.llm, originalLlmConfig);

    const logoutOutput = logoutLogs.join("\n");
    assert.match(logoutOutput, /Updated memory-lancedb-cip config: llm.auth=api-key/);
  });
  ...
  it("defaults the OAuth file to the plugin-scoped path under OPENCLAW_HOME", async () => {
    ...
    const configPath = path.join(tempDir, "openclaw.json");
    const oauthPath = path.join(process.env.OPENCLAW_HOME, ".memory-lancedb-cip", "oauth.json");
    const backupPath = getBackupPath(oauthPath);
    writeFileSync(configPath, JSON.stringify({
      plugins: {
        entries: {
  ...
  it("resolves stored relative oauthPath against the config location during logout", async () => {
    ...
    const storedOauthPath = ".memory-lancedb-cip/oauth.json";
    const actualOauthPath = path.join(workspaceDir, ".memory-lancedb-cip", "oauth.json");
  ...
  it("removes llm config on logout when only OAuth-generated fields remain and no backup exists", async () => {
    ...
    const configPath = path.join(workspaceDir, "openclaw.json");
    const oauthPath = path.join(workspaceDir, ".memory-lancedb-cip", "oauth.json");
