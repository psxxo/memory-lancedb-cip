...
function resolveDefaultOauthPath(): string {
  return path.join(resolveOpenClawHome(), ".memory-lancedb-cip", "oauth.json");
}
...
function safeObsidianSlug(value: string, fallback = "memory"): string {
  const slug = value
    .normalize("NFKD")
    .replace(/[\\/:*?"<>|#^[\]]+/g, " ")
    .replace(/\s+/g, "-")
...
function memoryDate(memory: MemoryEntry): string {
...
function describeImportValue(value: unknown): string {
 ...
 */
function validateImportEnvelope(data: unknown): { memories: unknown[]; error?: string } {
  if (data === null) {
...
function formatObsidianMemory(memory: MemoryEntry): string {
  ...
  const frontmatter = [
    ...
    `source: ${yamlScalar("memory-lancedb-cip")}`,
...
export async function runImportMarkdown(
  ...
  let explicitMarkdownFile: string | undefined;
  ...
  for (const { filePath, scope: discoveredScope } of mdFiles) {
    ...
    if (!options.dryRun) {
      if (pendingEntries.length > 0) {
        console.log(
          `  [import] ${pendingEntries.length} entries need embedding from ${path.basename(filePath)} ` +
...
export function registerMemoryCLI(program: Command, context: CLIContext): void {
  ...
  auth
    ...
    .option("--oauth-path <path>", "OAuth file path (default: ~/.openclaw/.memory-lancedb-cip/oauth.json)")
    ...
    .action(async (options) => {
      try {
        const pluginId = context.pluginId || "memory-lancedb-cip";
      ...
      try {
        ...
        try {
          ...
          throw new ImportFileError(
            `Unable to read import file "${file}" (${(readError as Error).message}).`,
          );
        }
        ...
        try {
          ...
          throw new ImportFileError(`Found invalid JSON (${(parseError as Error).message}).`);
        }
        ...
        for (const [index, rawMemory] of importMemories.entries()) {
          try {
            ...
            if (rawMemory === null || typeof rawMemory !== "object" || Array.isArray(rawMemory)) {
              ...
              skipped++;
              console.warn(
                `  memories[${index}]: expected an object, found ${describeImportValue(rawMemory)}; skipped.`,
              );
              continue;
            }
            ...
            if (typeof text !== "string" || text.trim().length === 0) {
              ...
              skipped++;
              console.warn(
                `  memories[${index}]: missing "text" (expected a non-empty string); skipped.`,
              );
              continue;
            }
            if (text.length < 2) {
  ...
  memory
    ...
    .description(
      "Import memories from Markdown files (MEMORY.md, memory/YYYY-MM-DD.md) into the plugin store; " +
  ...
  memory
    .command("doctor")
    .description(
      "Diagnose store health: dbPath, write-lock state and age, row counts, FTS/index status, and LanceDB version-dir health",
    )
    .option("--json", "Emit machine-readable JSON instead of the human report", false)
    .option(
      "--quarantine-corrupt",
      "If a structurally corrupt memories.lance is detected, rename it to memories.lance.corrupt-<UTC> (never deletes data) and start empty",
      false,
    )
    .action(async (options: { json: boolean; quarantineCorrupt: boolean }) => {
      try {
        ...
        const ms = (value: number | null) => (value === null ? "n/a" : `${Math.max(0, Math.round(value))}ms`);
