...
| **Умное извлечение** | Классификация на основе LLM по 10 категориям: `profile`, `preferences`, `entities`, `events`, `cases`, `patterns`, `decision`, `fact`, `reflection`, `other` |
...
3. Проверьте: `openclaw plugins info memory-lancedb-cip && openclaw memory-cip stats`
...
| `cli.ts` | CLI-команды: `memory-cip list/search/stats/delete/delete-bulk/export/import/reembed/upgrade/migrate` |
...
| `src/smart-extractor.ts` | Извлечение по 10 категориям на базе LLM с многослойным хранением L0/L1/L2 и двухэтапной дедупликацией |
...
> `memory_search` / `memory_get` регистрируются как алиасы совместимости только пока эти имена свободны, поэтому они никогда не конфликтуют с инструментами памяти, уже предоставляемыми встроенной средой памяти.
...
- **Извлечение по 10 категориям на базе LLM**: `profile`, `preferences`, `entities`, `events`, `cases`, `patterns`, `decision`, `fact`, `reflection`, `other` — единый словарь; каноническое имя — это то, что сохраняется в столбце `category`. Алиасы в единственном числе (`preference`, `entity`, `event`, `case`, `pattern`) принимаются на входе, а неизвестные имена отклоняются. Для JSON-импорта оператор может задать явную политику для неизвестных имён (`--unknown reject|other|<canonical>`, по умолчанию `reject`) и сопоставить произвольные входные имена с каноническими категориями (`--category-map`).
...
- **Слияние с учётом категории**: `profile` всегда сливается; `preferences` / `entities` / `patterns` / `fact` / `reflection` сливаются при обнаружении дубликатов; `events` / `cases` / `decision` — только добавление (никогда не сливаются)
...
| **Умное извлечение LLM (10 категорий)** | - | Yes (v1.1.0) |
...
`--category` принимает канонические 10 категорий (`profile` / `preferences` / `entities` / `events` / `cases` / `patterns` / `decision` / `fact` / `reflection` / `other`) и входные алиасы (`preference` / `entity` / `event` / `case` / `pattern`). Неизвестные значения отклоняются с ошибкой валидации, а не молча откатываются к `patterns` или `other`.

**Политика категорий для `import` (ничто никогда не приводится молча).** Для каждой строки категория разрешается в таком порядке:

1. **точное каноническое имя** — сохраняется как есть;
2. **встроенный алиас** (`preference` → `preferences`, `entity` → `entities`, `event` → `events`, `case` → `cases`, `pattern` → `patterns`);
3. **`--category-map <file>`** — JSON-объект, сопоставляющий произвольные входные имена с каноническими категориями, например `{"lemmas":"cases"}`;
4. **`--unknown <policy>`** — определяет оставшиеся, нераспознанные имена:
   - `reject` (**по умолчанию**): пропускает строку и выводит предупреждение для каждой строки со списком канонических имён;
   - `other`: сохраняет строку как `other` — только потому, что оператор явно это запросил;
   - любое каноническое имя категории: сохраняет строку как эту категорию.

Каждое решение сообщается по строкам, а `--dry-run` выводит полный план разрешения (запрошенное значение → `canonical` / `aliased` / `mapped` / `other` / `rejected` → итоговая категория) до любой записи, поэтому политику можно безопасно итеративно уточнять. Нераспознанное значение `--unknown` или значение `--category-map`, не являющееся канонической категорией/алиасом, приводит к сбою команды вместо импорта чего-либо.

```bash
# Предпросмотр того, как разрешится категория каждой строки; ничего не сохраняет.
openclaw memory-cip import memories.json --dry-run

# Явно направить два известных неканонических имени, всё остальное отклонить.
openclaw memory-cip import memories.json --category-map map.json --unknown reject

# То же, но любое другое нераспознанное имя явно поместить в "other".
...
```markdown
## Правило 1 — Сохранение урока в двух записях
Каждая ошибка/урок → НЕМЕДЛЕННО сохранить ДВЕ записи памяти (обе — канонические категории в едином словаре из 10 категорий):
...
| `category` | string | Категория хранения (каноническая): `profile` / `preferences` / `entities` / `events` / `cases` / `patterns` / `decision` / `fact` / `reflection` / `other` |
...
> **Примечание о категориях:** существует **единый словарь из 10 категорий** — `profile`, `preferences`, `entities`, `events`, `cases`, `patterns`, `decision`, `fact`, `reflection`, `other`. Каноническое имя *и есть* то, что хранит поле верхнего уровня `category` (тождественное отображение; отдельного двойного слоя «семантика/хранилище» нет). На входе принимаются алиасы в единственном числе `preference` → `preferences`, `entity` → `entities`, `event` → `events`, `case` → `cases` и `pattern` → `patterns`; `decision`, `fact`, `reflection` и `other` сами являются каноническими. Неизвестные имена категорий **отклоняются** с ошибкой валидации — они никогда не сопоставляются молча с `patterns` или `other`.
>
> Поведение слияния / таймлайна / персистентности: `profile` всегда сливается (без таймлайна, персистентно); `preferences`, `entities` и `fact` сливаются и версионируются во времени через `fact_key` (персистентно); `patterns` и `reflection` сливаются без таймлайна (персистентно); `events` — только добавление (без таймлайна, персистентно, с оценкой вымысла); `cases` и `decision` — только добавление (без таймлайна, персистентно); `other` не сливается и не использует таймлайн и не персистентно.
...
|---------|-------------|
| **Умное извлечение** | Извлечение по 10 категориям на базе LLM с метаданными L0/L1/L2. При отключении откатывается к регулярным правилам. |
...
Полный список: [Участники](https://github.com/psxxo/memory-lancedb-cip/graphs/contributors)
