...
import { existsSync, accessSync, constants, mkdirSync, readdirSync, realpathSync, lstatSync, unlinkSync, writeFileSync, } from "node:fs";
import { access as accessAsync, lstat as lstatAsync, mkdir as mkdirAsync, readFile as readFileAsync, realpath as realpathAsync, rename as renameAsync, rmdir as rmdirAsync, stat as statAsync, unlink as unlinkAsync, writeFile as writeFileAsync, } from "node:fs/promises";
...
function resolveBoundedMs(raw, fallback, min, max) {
...
export function resolveWriteLockTimeoutMs(config) {
    ...
    const candidate = Number.isFinite(fromEnv) ? fromEnv : config.writeLockTimeoutMs;
    return resolveBoundedMs(candidate, DEFAULT_WRITE_LOCK_TIMEOUT_MS, MIN_WRITE_LOCK_TIMEOUT_MS, MAX_WRITE_LOCK_TIMEOUT_MS);
}
/** Resolve when contention progress warnings start. */
export function resolveWriteLockWarnAfterMs(config) {
    const envRaw = process.env.MEMORY_LANCEDB_WRITE_LOCK_WARN_AFTER_MS;
    const fromEnv = envRaw === undefined || envRaw === "" ? undefined : Number(envRaw);
    const candidate = Number.isFinite(fromEnv) ? fromEnv : config.writeLockWarnAfterMs;
    return resolveBoundedMs(candidate, WRITE_LOCK_WARN_AFTER_MS, 100, 300_000);
}
/** Resolve the store-open ceiling (connect + table open). */
export function resolveOpenTimeoutMs(config) {
    const envRaw = process.env.MEMORY_LANCEDB_OPEN_TIMEOUT_MS;
    const fromEnv = envRaw === undefined || envRaw === "" ? undefined : Number(envRaw);
    const candidate = Number.isFinite(fromEnv) ? fromEnv : config.openTimeoutMs;
...
export function resolveIndexCatchUpEnabled(config) {
    const envRaw = (process.env.MEMORY_LANCEDB_INDEX_CATCHUP ?? "").trim();
    if (/^(0|false|no|off)$/i.test(envRaw))
        return false;
    if (/^(1|true|yes|on)$/i.test(envRaw))
        return true;
    return config.indexCatchUp !== false;
}
/** Resolve the ceiling for a single index fold. */
export function resolveIndexCatchUpTimeoutMs(config) {
    const envRaw = process.env.MEMORY_LANCEDB_INDEX_CATCHUP_TIMEOUT_MS;
    const fromEnv = envRaw === undefined || envRaw === "" ? undefined : Number(envRaw);
    const candidate = Number.isFinite(fromEnv) ? fromEnv : config.indexCatchUpTimeoutMs;
    return resolveBoundedMs(candidate, DEFAULT_INDEX_CATCHUP_TIMEOUT_MS, 1_000, 3_600_000);
}
function resolveQuarantineCorruptEnabled(raw) {
 ...
 */
export async function describeLockContention(lockArtifactPath, ownerPath) {
    const lines = [];
    try {
        const stat = await statAsync(lockArtifactPath);
        const ageMs = Date.now() - stat.mtimeMs;
        lines.push(`Lock artifact: ${lockArtifactPath} (age=${formatDurationMs(ageMs)}, ` +
    ...
    try {
        const parsed = JSON.parse(await readFileAsync(ownerPath, "utf8"));
        let ownerAgeMs = null;
        try {
            ownerAgeMs = Date.now() - (await statAsync(ownerPath)).mtimeMs;
        }
        catch { }
        lines.push(`Suspected holder: pid=${typeof parsed.pid === "number" ? parsed.pid : "unknown"} ` +
 ...
 */
export function assessMemoryTableStructure(dbPath) {
...
function safeReadDirNames(path) {
    try {
...
function corruptQuarantineTargetName(tableDir, at = new Date()) {
...
export class MemoryStore {
    ...
    corruptQuarantineOverride = null;
        ...
        while (release === null) {
            try {
                ...
                if (Date.now() >= waitDeadline) {
                    throw await this.buildLockWaitTimeoutError(lockPath, lockArtifactPath, ownerPath, waitedMs, writeLockTimeoutMs);
                }
                if (waitedMs >= writeLockWarnAfterMs && Date.now() - lastWarnAt >= writeLockWarnIntervalMs) {
                    lastWarnAt = Date.now();
                    console.warn(`[memory-lancedb-cip] still waiting for the memory write lock after ${formatDurationMs(waitedMs)} ` +
     ...
     */
    async buildLockWaitTimeoutError(lockPath, lockArtifactPath, ownerPath, waitedMs, timeoutMs) {
        const err = new Error(`Timed out after ${formatDurationMs(waitedMs)} waiting for the memory write lock.\n` +
            `  Lock: ${lockPath}\n` +
        ...
        err.code = WRITE_LOCK_TIMEOUT_CODE;
        return err;
    }
    /**
    ...
    async foldIndices(reason) {
        ...
        console.log(`[memory-lancedb-cip] index fold starting (reason=${reason}, modsSinceLast=${mods}, timeout=${formatDurationMs(foldTimeoutMs)})`);
        try {
            await this.runWithWriteLock(async () => {
                await withTimeout(this.table.optimize({ cleanupOlderThan: new Date(0) }), foldTimeoutMs, `index fold (reason=${reason}) at "${this.config.dbPath}" did not return`);
            });
            console.log(`[memory-lancedb-cip] index fold completed (reason=${reason}, modsSinceLast=${mods}, elapsed=${formatDurationMs(Date.now() - startedAt)})`);
        }
        catch (err) {
            // Re-arm the counter so a transient failure retries on later writes.
            this.dataModsSinceIndexFold += mods;
            console.warn(`[memory-lancedb-cip] index fold failed (reason=${reason}, elapsed=${formatDurationMs(Date.now() - startedAt)}): ${err instanceof Error ? err.message : String(err)}`);
        }
        finally {
    ...
    async scheduleStartupIndexCatchUp() {
        if (!resolveIndexCatchUpEnabled(this.config)) {
            console.log("[memory-lancedb-cip] FTS catch-up fold skipped (storage.indexCatchUp=false / MEMORY_LANCEDB_INDEX_CATCHUP=0)");
            return;
        }
        try {
            const table = this.table;
            ...
            if (backlog >= MemoryStore.INDEX_FOLD_OP_THRESHOLD) {
                console.log(`[memory-lancedb-cip] FTS index has ${backlog} unindexed rows; scheduling background catch-up fold ` +
    ...
    async doInitialize() {
        ...
        const logStage = (message) => console.log(`[memory-lancedb-cip] ${message} (+${formatDurationMs(Date.now() - initStartedAt)})`);
        ...
        try {
            ...
            db = await withTimeout(lancedb.connect(this.config.dbPath, {
                readConsistencyInterval: this.config.readConsistencyInterval,
            }), openTimeoutMs, `LanceDB connect at "${this.config.dbPath}" did not return`);
            ...
            table = await withTimeout(this.openTableWithCorruptionRecovery(db), openTimeoutMs, `opening table "${TABLE_NAME}" at "${this.config.dbPath}" did not return`);
        ...
        logStage(`ready in ${formatDurationMs(this.initDurationMs)} ` +
     ...
     */
    async hasLegacySecondTimestampWork(table) {
        try {
            const candidateRows = await table.query()
                ...
                .toArray();
            return candidateRows.some((row) => isLegacySecondTimestamp(row.timestamp) ||
     ...
     */
    async createFtsIndexIfMissing(table) {
        if (await this.hasFtsIndexRow(table))
            return;
        await this.createFtsIndexWithWriteLock(table);
    }
    /** Readable open failure: path + reason + suggested commands, never a hang. */
    buildStoreOpenError(err, openTimeoutMs) {
        const original = err instanceof Error ? err : new Error(String(err));
        const code = original.code ?? "";
        const structure = assessMemoryTableStructure(this.config.dbPath);
        const enhanced = new Error(`Failed to open LanceDB store at "${this.config.dbPath}": ${code} ${original.message}\n` +
     ...
     */
    async openTableWithCorruptionRecovery(db) {
        try {
            ...
            if (!quarantineEnabled) {
                console.error(`[memory-lancedb-cip] STRUCTURAL CORRUPTION DETECTED — the store will NOT be modified.\n` +
            ...
            console.error(`[memory-lancedb-cip] STRUCTURAL CORRUPTION DETECTED — quarantining (NO DATA DELETED).\n` +
                `  Table dir: ${structure.tableDir}\n` +
                `  Reason: ${structure.detail}\n` +
                `  Original error: ${err instanceof Error ? err.message : String(err)}\n` +
            ...
            try {
                ...
                console.error(`[memory-lancedb-cip] quarantine rename failed, original table left untouched: ` +
    ...
    async diagnose() {
        ...
        if (!dbPathExists)
            warnings.push(`dbPath does not exist yet: ${dbPath}`);
        else if (!dbPathWritable)
            warnings.push(`dbPath is not writable: ${dbPath}`);
        let artifactExists = false;
        let artifactAgeMs = null;
        ...
        let ownerHintAgeMs = null;
        try {
            const parsed = JSON.parse(await readFileAsync(ownerPath, "utf8"));
            if (typeof parsed.pid === "number")
                ownerPid = parsed.pid;
            if (typeof parsed.host === "string")
                ownerHost = parsed.host;
            if (typeof parsed.startedAt === "string")
                ownerStartedAt = parsed.startedAt;
            try {
        ...
        let indexName = null;
        let unindexedRows = null;
        let indexCount = null;
        if (this.table && typeof this.table.listIndices === "function") {
            try {
                const indices = await this.table.listIndices();
                const list = Array.isArray(indices) ? indices : [];
                indexCount = list.length;
                const fts = list.find((idx) => idx?.indexType === "FTS" || idx?.columns?.includes("text"));
                if (fts) {
                    indexName = fts.name ?? "text";
        ...
        if (structure.structurallyCorrupt) {
            warnings.push(`structural corruption: ${structure.detail}. No data was modified. ` +
                `Quarantine by rename (never delete): mv "${structure.tableDir}" "${corruptQuarantineTargetName(structure.tableDir)}"`);
        }
        if (artifactStale) {
            warnings.push(`write-lock artifact looks stale (age ${formatDurationMs(artifactAgeMs ?? 0)} > ${WRITE_LOCK_STALE_MS}ms): ${artifactPath}. ` +
    ...
    async fetchForCompaction(maxTimestamp, scopeFilter, limit = 200, options) {
        ...
        const ranked = lightRows
            ...
            .filter((row) => excludeInactive ? isMemoryActiveAt(parseSmartMetadata(row.metadata, { id: row.id })) : true)
