...
export function normalizeOAuthProviderId(providerId) {
    const raw = providerId?.trim().toLowerCase();
    if (!raw)
        return DEFAULT_OAUTH_PROVIDER_ID;
...
export function getOAuthProviderLabel(providerId) {
    return getOAuthProvider(providerId).label;
}
export function getDefaultOauthModelForProvider(providerId) {
    return getOAuthProvider(providerId).defaultModel;
}
export function isOauthModelSupported(providerId, value) {
    if (!value || !value.trim())
        return false;
...
function getJwtAccountId(token, providerId) {
    const provider = getOAuthProvider(providerId);
    const payload = decodeJwtPayload(token);
...
function pickString(container, keys) {
...
function pickTimestamp(container, keys) {
    for (const key of keys) {
...
function extractSessionFromObject(source, authPath) {
    const scopes = [
        source,
        typeof source.tokens === "object" && source.tokens ? source.tokens : {},
        typeof source.oauth === "object" && source.oauth ? source.oauth : {},
        typeof source.openai === "object" && source.openai ? source.openai : {},
        typeof source.chatgpt === "object" && source.chatgpt ? source.chatgpt : {},
        typeof source.auth === "object" && source.auth ? source.auth : {},
        typeof source.credentials === "object" && source.credentials ? source.credentials : {},
    ];
    let accessToken;
    let refreshToken;
    let expiresAt;
...
export async function loadOAuthSession(authPath) {
    let raw;
    try {
...
async function exchangeAuthorizationCode(code, verifier, providerId) {
...
export async function performOAuthLogin(options) {
...
export function normalizeOauthModel(model) {
...
export function buildOauthEndpoint(baseURL, providerId) {
    const root = (baseURL?.trim() || getOAuthProvider(providerId).backendBaseUrl).replace(/\/+$/, "");
    if (root.endsWith("/codex/responses"))
        return root;
    if (root.endsWith("/responses"))
        return root.replace(/\/responses$/, "/codex/responses");
    return `${root}/codex/responses`;
}
function extractOutputTextFromResponsePayload(payload) {
...
export function extractOutputTextFromSse(bodyText) {
