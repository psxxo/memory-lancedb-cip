...
function combineIdentityTextProbe(params) {
    return [
        params.text,
        params.abstract,
        params.overview,
...
function makeCandidate(kind, alias, sourceText) {
    if (kind === "name") {
        return {
            category: "entities",
            abstract: `姓名：${alias}`,
...
export function createIdentityAndAddressingCandidates(text) {
...
export function extractIdentityAndAddressingValues(text) {
    const sourceText = text.trim();
    if (!sourceText)
        return {};
    return {
...
export function classifyIdentityAndAddressingMemory(params) {
    const slots = new Set();
...
export function canonicalizeIdentityAndAddressingCandidate(candidate) {
...
export function isCanonicalIdentityOrAddressingFactKey(factKey) {
