export function extractSectionMarkdown(markdown, heading) {
    ...
    for (const raw of lines) {
        ...
        if (lower.startsWith("## ")) {
            if (inSection && lower !== headingNeedle)
                break;
...
export function isPlaceholderReflectionSliceLine(line) {
    ...
    if (!normalized)
        return true;
    if (/^\(none( captured)?\)$/i.test(normalized))
        return true;
    if (/^(invariants?|reflections?|derived)[:：]$/i.test(normalized))
        return true;
    if (/apply this session'?s deltas next run/i.test(normalized))
        return true;
    if (/apply this session'?s distilled changes next run/i.test(normalized))
        return true;
    if (/investigate why embedded reflection generation failed/i.test(normalized))
        return true;
    return false;
}
...
export function extractReflectionLearningGovernanceCandidates(reflectionText) {
    ...
    if (parsed.length > 0)
        return parsed;
    const fallbackBullets = sanitizeReflectionSliceLines(parseSectionBullets(reflectionText, "Learning governance candidates (.learnings / promotion / skill extraction)"));
...
function extractReflectionMappedMemoryItemsWithSanitizer(reflectionText, sanitizeLines) {
...
function extractReflectionSlicesWithSanitizer(reflectionText, sanitizeLines) {
    ...
    const invariantLinesLegacy = sanitizeLines(mergedSection.filter((line) => /invariant|stable|policy|rule/i.test(line))).filter(isInvariantRuleLike);
    const reflectionLinesLegacy = sanitizeLines(mergedSection.filter((line) => /reflect|inherit|derive|change|apply/i.test(line))).filter(isDerivedDeltaLike);
