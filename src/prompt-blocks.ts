...
export const CATEGORY_TAXONOMY =
  "The memory system stores ten categories: profile (user identity), preferences (user tendencies), entities (long-lived project/entity state), events (one-off things that happened), cases (problem + solution pairs), patterns (reusable procedures), decision (durable decisions and commitments), fact (durable factual knowledge), reflection (the assistant's distilled self-model notes), and other (the non-durable catch-all for anything that fits nothing else).";
 ...
 */
function formatFieldLines(label: string, value: string): string[] {
...
export function formatExistingMemoryEntry(
  n: number,
  category: string,
  abstract: string,
  score: number,
...
export function formatExistingMemoriesSection(entries: string[]): string {
