...
> => {
  ...
  try {
    ...
    throw new Error(
      `memory-lancedb-cip: failed to load LanceDB. ${String(err)}`,
      { cause: err },
    );
  }
};
...
export class MemoryStore {
  ...
  private async runWithFileLock<T>(fn: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (isCompromised) {
        ...
        console.warn(
          `[memory-lancedb-cip] Returning successful result despite compromised lock at "${lockPath}". ` +
  ...
  private async closeLockResources(): Promise<void> {
    ...
    try {
      ...
      this.config.onLockWarning?.(
        `memory-lancedb-cip: failed to close Redis lock client: ${String(err)}`,
      );
    }
  }
  ...
  private async doInitialize(): Promise<void> {
    ...
    try {
      ...
      this.config.onStoragePathWarning?.(
        `memory-lancedb-cip: storage path issue — ${String(err)}\n` +
  ...
  private async migrateLegacyTableColumns(table: LanceDB.Table, alreadyLocked = false): Promise<void> {
    try {
      ...
      const applyMigration = async () => {
        ...
        console.warn(
          `memory-lancedb-cip: migrating legacy table — adding columns: ${currentMissingColumns.map((c) => c.name).join(", ")}`,
        );
        await table.addColumns(currentMissingColumns);
        console.log(
          `memory-lancedb-cip: migration complete — ${currentMissingColumns.length} column(s) added`,
        );
      };
  ...
  async vectorSearch(vector: number[], limit = 5, minScore = 0.3, scopeFilter?: string[], options?: { excludeInactive?: boolean }): Promise<MemorySearchResult[]> {
    ...
    if (this.disableNativeCosine && !this.nativeCosineFallbackLogged) {
      console.warn(
        "memory-lancedb-cip: LanceDB native vector cosine disabled; scanning candidates and using JS cosine rerank fallback",
      );
