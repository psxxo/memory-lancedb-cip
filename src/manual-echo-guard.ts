...
function orderedContentTokens(normalized: string): string[] {
  ...
  for (const token of tokens) {
    ...
    if (SPEAKER_TOKENS.has(token)) continue;
    if (REFERENT_PRONOUNS.has(token)) {
      if (userWrapped) continue;
      out.push(token);
      continue;
    }
    if (ECHO_GLUE.has(token) || ECHO_RELATION_TOKENS.has(token)) continue;
    out.push(token);
  }
  return out;
}

function orderedRelationTokens(normalized: string): string[] {
 ...
 */
function relationTokensAgree(candidateRelations: string[], manualRelations: string[]): boolean {
  let m = 0;
  for (const token of candidateRelations) {
    ...
    if (!WRAP_INSERTABLE_COPULAS.has(token)) return false;
  }
...
export function isNearIdenticalEcho(candidateText: string, manualText: string): boolean {
  ...
  if (
    candidateContentList.length >= MIN_CONTAINMENT_TOKENS &&
  ...
  if (!candidateContentList.every((token, i) => token === manualContentList[i])) return false;
