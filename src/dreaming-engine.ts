...
export type DreamingPhaseName = "light" | "deep" | "rem";
...
export interface DreamingConfig {
  ...
  phases: {
    light: LightDreamingConfig;
    deep: DeepDreamingConfig;
    rem: RemDreamingConfig;
  };
}
...
export interface DreamingSweepResult {
  ...
  finishedAt: number;
  scopes: string[];
  phases: Record<DreamingPhaseName, DreamingPhaseResult>;
  errors: Array<{ scope: string; phase: DreamingPhaseName; message: string }>;
}

export interface DreamingPattern {
  type: "category" | "memory_category" | "term";
  key: string;
  count: number;
  strength: number;
}
...
type DreamingStore = {
  ...
  update(id: string, updates: MemoryUpdatePatch, scopeFilter?: string[]): Promise<MemoryEntry | null>;
  store(entry: Omit<MemoryEntry, "id" | "timestamp">): Promise<MemoryEntry>;
  stats(scopeFilter?: string[]): Promise<{ totalCount: number; scopeCounts: Record<string, number> }>;
};

type DreamingEmbedder = {
...
export interface DreamingEngineDependencies {
  store: DreamingStore;
  embedder: DreamingEmbedder;
  decayEngine?: DecayEngine;
  tierManager?: TierManager;
  config?: PartialDreamingConfig;
  getScopes?: () => Promise<string[]>;
  logger?: DreamingLogger;
  now?: () => number;
}

export type PartialDreamingConfig = Partial<Omit<DreamingConfig, "phases">> & {
  phases?: {
    light?: Partial<LightDreamingConfig>;
    deep?: Partial<DeepDreamingConfig>;
    rem?: Partial<RemDreamingConfig>;
  };
};
...
function normalizeLight(raw: unknown): LightDreamingConfig {
...
function normalizeDeep(raw: unknown): DeepDreamingConfig {
  const cfg = isRecord(raw) ? raw : {};
  return {
    enabled: cfg.enabled !== false,
...
function normalizeRem(raw: unknown): RemDreamingConfig {
  const cfg = isRecord(raw) ? raw : {};
  return {
    enabled: cfg.enabled !== false,
...
export function parseDailyCron(value: string | undefined): { minute: number; hour: number } | null {
...
export function computeNextDreamingDelayMs(
...
function emptyPhaseResult(): DreamingPhaseResult {
...
function selectCanonical(a: MemoryEntry, b: MemoryEntry): MemoryEntry {
...
function parseUniqueQueryCount(metadata: Record<string, unknown>): number {
...
function addPatternCount(counts: Map<string, number>, key: string | undefined) {
  if (!key) return;
  counts.set(key, (counts.get(key) ?? 0) + 1);
}

function buildPatterns(entries: MemoryEntry[], minPatternStrength: number): DreamingPattern[] {
  ...
  const build = (type: DreamingPattern["type"], counts: Map<string, number>): DreamingPattern[] =>
    [...counts.entries()]
...
function formatDateStamp(now: number, timezone?: string): string {
  const parts = getZonedParts(now, timezone);
  return `${parts.year.toString().padStart(4, "0")}-${parts.month.toString().padStart(2, "0")}-${parts.day.toString().padStart(2, "0")}`;
}

function phaseEnabled(config: { enabled: boolean; limit?: number }): boolean {
...
export function createDreamingEngine(deps: DreamingEngineDependencies): DreamingEngine {
  ...
  async function resolveScopes(explicit?: string[]): Promise<string[]> {
  ...
  async function runLight(scope: string): Promise<DreamingPhaseResult> {
  ...
  async function runDeep(scope: string): Promise<DreamingPhaseResult> {
    ...
    for (const entry of entries) {
      ...
      if (metadata.access_count < phase.minRecallCount) continue;
  ...
  async function runRem(scope: string): Promise<DreamingPhaseResult> {
